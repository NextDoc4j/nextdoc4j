 ...
 */
public class Jackson3DocJsonMapper implements DocJsonMapper {

    private final ObjectMapper objectMapper;

    public Jackson3DocJsonMapper() {
        this(new ObjectMapper());
    }

    public Jackson3DocJsonMapper(ObjectMapper objectMapper) {
    ...
    public DocObjectNode createObjectNode() {
        return new J3Object(objectMapper.createObjectNode());
    }

    @Override
    public DocArrayNode createArrayNode() {
        return new J3Array(objectMapper.createArrayNode());
    }
    ...
    private static DocJsonNode wrap(JsonNode node) {
        ...
        if (node instanceof ObjectNode objectNode) {
            return new J3Object(objectNode);
        }
        if (node instanceof ArrayNode arrayNode) {
            return new J3Array(arrayNode);
        }
        return new J3Node(node);
    }
    ...
    private static JsonNode unwrap(DocJsonNode node) {
        if (node instanceof J3Node j3Node) {
            return j3Node.delegate;
        }
        throw new IllegalArgumentException("Not a Jackson3 node: " + node);
    }

    private static class J3Node implements DocJsonNode {
        final JsonNode delegate;

        J3Node(JsonNode delegate) {
        ...
        public DocObjectNode asObject() {
            return delegate instanceof ObjectNode objectNode ? new J3Object(objectNode) : null;
        }

        @Override
        public DocArrayNode asArray() {
            return delegate instanceof ArrayNode arrayNode ? new J3Array(arrayNode) : null;
        }
    ...
    private static final class J3Object extends J3Node implements DocObjectNode {
        private final ObjectNode object;

        J3Object(ObjectNode object) {
            super(object);
            this.object = object;
        }
        ...
        public DocObjectNode withObject(String field) {
            return new J3Object(object.withObject(field));
        }
        ...
        public void forEachField(BiConsumer<String, DocJsonNode> consumer) {
            object.properties().forEach(entry -> consumer.accept(entry.getKey(), wrap(entry.getValue())));
        }
    }

    private static final class J3Array extends J3Node implements DocArrayNode {
        private final ArrayNode array;

        J3Array(ArrayNode array) {
            super(array);
            this.array = array;
        }
        ...
        public DocArrayNode deepCopyArray() {
            return new J3Array(array.deepCopy());
        }
    }
}
