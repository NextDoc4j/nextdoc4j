...
* **后端**
  ...
  * 修复同时注册多个 `RequestMappingHandlerMapping`（如 Spring Boot Actuator）时生成 OpenAPI 文档抛出 `NoUniqueBeanDefinitionException` 的问题。
  * 修复多映射器场景下 `@SaIgnore` 排除路径无法正确收集的问题。
* **前端**

  * 修复树结构示例嵌套过深的问题，限制自引用结构展开层级并收束叶子节点。
  * 修复循环引用类型展示为 `array<ref>` 的问题，改为展示实际组件类型名称。
  * 修复非字符串类型的空字符串默认值导致请求体或子节点示例为空的问题。
...
* **构建**
  * 修复多模块引入内部 BOM 和核心依赖时发布 POM 版本未固定的问题，统一使用 revision 声明并通过 flatten 生成固定版本。
