...
public class GatewayRouteDocProvider {
    ...
    public List<AbstractSwaggerUiConfigProperties.SwaggerUrl> getManualConfiguredUrls() {
        return getManualConfiguredDocEntries().stream().map(this::convertToSwaggerUrl).toList();
    }
    ...
    public List<GatewayDocRouteEntry> getManualConfiguredDocEntries() {
        ...
        return services.stream().filter(this::isValidServiceConfig).map(this::convertServiceConfigToDocEntry).toList();
    }
