...
public class NextDoc4jEnumParameterHandler implements ParameterCustomizer, ModelConverter {
    ...
    /**
     * 宿主 ObjectMapper（Jackson 2 或 3），经反射调用，避免 compile 绑定具体包名。
     */
     ...
     * @param objectMapper 宿主 ObjectMapper 实例
     */
    public NextDoc4jEnumParameterHandler(List<EnumMetadataResolver> resolvers, Object objectMapper) {
    ...
    private void configureEnumSchema(Schema<?> schema, Class<?> enumClass) {
        ...
        if (enumValues != null && !enumValues.isEmpty()) {
            // swagger Schema<?>#setEnum 签名为原始/捕获通配 List，无法在无 raw 的情况下完全类型安全赋值
            setSchemaEnumValues(schema, enumValues);
        }
        ...
        if (metadata != null && metadata.getItems() != null && !metadata.getItems().isEmpty()) {
            Map metadataMap = convertValueToMap(metadata);
            if (metadataMap != null) {
     ...
     * 写入 schema.enum 列表。
     * <p>
     * swagger-core 的 {@link Schema#setEnum(List)} 在 {@code Schema<?>} 上无法完全泛型安全，
     * 故在本方法内收窄 raw/unchecked 抑制范围。
     *
     * @param schema     目标 Schema
     * @param enumValues 枚举值列表（非空）
     */
    @SuppressWarnings({"rawtypes", "unchecked"})
    private static void setSchemaEnumValues(Schema<?> schema, List<?> enumValues) {
        Schema raw = schema;
        raw.setEnum(new java.util.ArrayList<>(enumValues));
    }
    ...
    private Class<?> resolveRawClass(AnnotatedType type) {
        ...
        java.lang.reflect.Type sourceType = type.getType();
        if (sourceType instanceof Class<?> clazz) {
            return clazz;
        }
        // Jackson JavaType 常带 getRawClass
        try {
            Object raw = sourceType.getClass().getMethod("getRawClass").invoke(sourceType);
        ...
        try {
            Object javaType = objectMapper.getClass()
                .getMethod("constructType", java.lang.reflect.Type.class)
                .invoke(objectMapper, sourceType);
            Object raw = javaType.getClass().getMethod("getRawClass").invoke(javaType);
    ...
    /**
     * 反射调用 ObjectMapper#convertValue，兼容 Jackson 2/3。
     */
    @SuppressWarnings({"rawtypes", "unchecked"})
    private Map convertValueToMap(Object value) {
        if (objectMapper == null || value == null) {
            return null;
        }
        try {
            return (Map)objectMapper.getClass()
                .getMethod("convertValue", Object.class, Class.class)
                .invoke(objectMapper, value, Map.class);
        } catch (ReflectiveOperationException e) {
            return null;
        }
    }
